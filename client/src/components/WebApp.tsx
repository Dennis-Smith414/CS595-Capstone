...
function flattenToLatLng(geom: GeoGeometry): [number, number][] {
  return geom.type === "LineString"
    ? geom.coordinates.map(toLatLng)
    : geom.coordinates.flatMap((seg) => seg.map(toLatLng));
}
...
function MapScreen({ selected }: { selected: number[] }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Clear previous
      layersRef.current.forEach((lyr) => map.removeLayer(lyr));
      layersRef.current = [];
      ...
      for (const id of selected) {
        try {
          const feat = await fetchRouteGeo(id);
          const coords = flattenToLatLng(feat.geometry);
          all.push(...coords);
          const poly = L.polyline(coords, { weight: 4, color: "#0ec1ac" }).addTo(map);
      ...
      if (all.length) {
        const bounds = (window as any).L.latLngBounds(all);
  ...
  return (
    <div style={{ position: "relative" }}>
      <div ref={ref} style={sx.map} />
      <div style={sx.mapBadge}>Loading route tracks…</div>
    </div>
  );
}

function AccountScreen({ onLogout }: { onLogout: () => void }) {
  // TODO: replace with /api/auth/me once available
